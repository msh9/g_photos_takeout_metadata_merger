...
def setup_arguments():
    ...
    parser.add_argument('duplicate_tracking', type=str, help='Path to an existing gzipped JSON file with seen file hashes created by this application')
    parser.add_argument('output_directory', type=str, help='Path to the output directory')
    return parser

def create_and_ensure_destination_path(root: Path, takeout_metadata: TakeoutMetadata, content_archive_path: Path) -> Path:
    photo_taken = takeout_metadata.get_photo_taken_time()
    content_destination = root.joinpath(str(photo_taken.year), str(photo_taken.month), content_archive_path.name)
    ...
    with Archive(*args.tarfiles) as archive:
        ...
        while True:
            ...
            if files_processed_counter % persist_seen_files_every == 0:
                logging.info(f"Processed {files_processed_counter} files, saving seen contents files storage to {args.duplicate_tracking}")
                seen_content.save()
