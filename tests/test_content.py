...
import pyexiv2
import tempfile
from photo_metadata_merger.exifio.content import (
    GenericXMPExifContent,
    GenericXMPContent,
    XMPSidecar,
)
...
class TestGenericXMPExifContent(unittest.TestCase):
    ...
    def test_process_content_updates_timestamps(self):
        with pyexiv2.ImageData(TestGenericXMPExifContent.test_fixture) as image_data:
            ...
            self.assertEqual(
                exif["Exif.Photo.DateTimeOriginal"], "2021-05-21 05:00:00+00:00"
            )
            self.assertEqual(
                exif["Exif.Photo.DateTimeDigitized"], "2023-05-22 19:34:53+00:00"
            )
...
class TestGenericXMPContent(unittest.TestCase):
    ...
    def test_process_content_updates_xmp_dates(self):
        with pyexiv2.ImageData(TestGenericXMPContent.test_fixture) as content:
            ...
            self.assertEqual(
                xmp["Xmp.exif.DateTimeOriginal"], "2021-05-21 05:00:00+00:00"
            )
            self.assertEqual(
                xmp["Xmp.exif.DateTimeDigitized"], "2023-05-22 19:34:53+00:00"
            )
