...
class TestArchive(unittest.TestCase):
    @staticmethod
    def _create_test_archive_from_directory(
        archive: tarfile.TarFile, directory_name: str
    ) -> None:
        cls_path = pathlib.Path(__file__).parent.absolute()
        for resource_to_be_tarred in cls_path.joinpath(directory_name).iterdir():
            TestArchive._add_file_with_base_name_only_to_archive(
                archive, resource_to_be_tarred
            )

    @staticmethod
    def _add_file_with_base_name_only_to_archive(
        tar: tarfile.TarFile, to_be_tarred: str
    ) -> None:
    ...
    def setUpClass(cls):
        ...
        cls.first_archive_path = pathlib.Path(
            TestArchive._archive_directory.name, "test1.tgz"
        )

        with tarfile.open(cls.first_archive_path, mode="w:gz") as archive:
            TestArchive._create_test_archive_from_directory(
                archive, constants.tarone_resource_directory
            )

        cls.second_archive_path = pathlib.Path(
            TestArchive._archive_directory.name, "test2.tgz"
        )

        with tarfile.open(cls.second_archive_path, mode="w:gz") as archive:
            TestArchive._create_test_archive_from_directory(
                archive, constants.tartwo_resource_directory
            )
    ...
    def test_archive_gets_next_image(self):
        with archive.Archive(
            TestArchive.first_archive_path, TestArchive.second_archive_path
        ) as pa:
    ...
    def test_archive_returns_correct_metadata_per_image(self):
        with archive.Archive(
            ...
            self.assertEqual(
                metadata_path, content_path.with_suffix(content_path.suffix + ".json")
            )
